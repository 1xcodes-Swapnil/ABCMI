...
export const MeetingsTester: React.FC<MeetingsTesterProps> = ({
      ...
      default:
        return (
          <span className="inline-flex items-center text-xs font-medium text-slate-500 dark:text-slate-400">
            <FileAudio className="w-3.5 h-3.5 mr-1" /> Ready for Intake
          </span>
        );
    }
  };
  ...
  return (
    <div id="meetings-tester-container" className="space-y-6">
      ...
      <AnimatePresence>
        {deleteToast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="p-3 rounded-lg bg-slate-900 border border-rose-800 text-rose-300 text-xs flex items-center justify-between shadow-lg"
          >
      ...
      {/* Top Hero Banner with Subtle Acoustic Waveform Artwork */}
      <div
        className={`rounded-xl border relative overflow-hidden transition-colors ${
          theme === 'dark'
      ...
      >
        ...
        <div className="absolute inset-0 opacity-15 pointer-events-none overflow-hidden">
          <img
            src="/src/assets/images/enterprise_audio_waveform_1790188184160.jpg"
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-slate-800/80 border border-slate-700/80 text-indigo-400">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    <span>Multi-Speaker Acoustic Ingestion</span>
                    <span aria-hidden="true">·</span>
                    <span>Max 500 MB WAV/MP3/M4A</span>
                    <span aria-hidden="true">·</span>
                    <span>Open-MOSS + PyAnnote 3.1</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`inline-flex items-center justify-center px-4 py-2 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${
                  isIntakeExpanded
                    ? 'bg-slate-800 border-slate-700 text-slate-200'
                    : 'bg-indigo-600 hover:bg-indigo-500 text-white border-indigo-600 shadow-sm active:scale-98'
                }`}
              >
                <Plus className={`w-3.5 h-3.5 mr-1.5 transition-transform duration-200 ${isIntakeExpanded ? 'rotate-45' : ''}`} />
                <span>{isIntakeExpanded ? 'Close Intake Form' : 'New Meeting'}</span>
              </button>
            </div>
          </div>

          {/* Quick Presets Segmented Bar */}
          <div className="mt-4 pt-3.5 border-t border-slate-200 dark:border-slate-800/80 flex flex-wrap items-center gap-2 text-xs">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              {PRESET_TEMPLATES.map((p, idx) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-md text-xs font-medium transition-colors bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800/60 dark:hover:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700/60 cursor-pointer"
                >
                  {p.name}
                </button>
              ))}
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div className="space-y-1 flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-base font-semibold text-slate-900 dark:text-white tracking-tight truncate">
                        {m.title}
                      </h3>
                  ...
                  <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer"
                    ...
                    {onViewIntelligence && m.status === 'completed' && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-950/60 dark:hover:bg-indigo-900/60 dark:text-indigo-200 border border-indigo-200 dark:border-indigo-800/80 text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer"
                    ...
                    {onDeleteMeeting && (
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded-lg text-slate-400 hover:text-rose-600 dark:text-slate-500 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 border border-transparent hover:border-rose-200 dark:hover:border-rose-900/50 text-xs font-medium flex items-center gap-1 transition-colors cursor-pointer group"
                        title="Remove this meeting session"
                      >
                        <Trash2 className="w-3.5 h-3.5 text-slate-400 group-hover:text-rose-600 dark:group-hover:text-rose-400" />
                        <span>Remove</span>
                      </button>
                    )}
                  </div>
